...
const ArtistName = () => {
  ...
  const onClick = async (text) => {
    const newAlbum = await new fetcher('post', 'album', { artistname: target.name, albumtitle: `${albumList.length} 앨범`, albumtype: "싱글", date: Date.now(), song: [], imgurl: '' })
    setAlbumList([...albumList, newAlbum])
  }

  const onDelete = (title) => {
    // const del = albumtList.filter(i => i.albumtitle === title)
    console.log(albumList.findIndex((i) => (i.albumtitle === title)))
  }
  const getArtist = async () => {
    const data = await fetcher('get', `artist/${query}`)
    setArtist(data)
    setAlbumList(data.targetAlbum)
  }
      ...
      {!load ?
        <>
          ...
          <section className={'default_section'}>
            {targetMember.length !== 0 ? <section className={'art_info_wrap'}>
              {targetMember.map(i =>
            ...
            <section className={'art_info_wrap'}>
              <IconTitle text={`발매 앨범 (${albumList.length}개)`} icon={'recent'} />
              <div>
                <h2 onClick={onClick}>앨범 추가</h2>
              </div>
              {albumList.sort((a, b) => {
                return b.date - a.date
              }).map(i => <Discography onDelete={() => onDelete(i.albumtitle)} {...i} key={i.albumtitle} />)}
            </section>
            <section className={'art_info_wrap'}>
